...
def car(xLen, yLen, zLen, xBL, yBL, zBL):

    glBegin(GL_QUADS)

    #bottom
    glVertex3d(xBL, yBL, zBL);
    glVertex3d(xBL, yLen + yBL, zBL);
    glVertex3d(xLen + xBL, yLen + yBL, zBL);
    glVertex3d(xLen + xBL, yBL, zBL);

    #left side wall
    glVertex3d(xBL, yBL, zBL);
    glVertex3d(xBL, yBL, zLen + zBL);
    glVertex3d(xBL, yLen + yBL, zLen + zBL);
    glVertex3d(xBL, yLen + yBL, zBL);

    #top side wall
    glVertex3d(xBL, yLen + yBL, zBL);
    glVertex3d(xBL, yBL + yLen, zBL + zLen);
...
# import pandas as pd
# df = pd.read_csv('pitch.obj', delimiter=' ')
# glBegin(GL_TRIANGLES)
# for i in range(len(df['Letter'])):
#     glVertex3d(df.loc[i,'X'], df.loc[i,'Y'], df.loc[i,'Z'])
# glEnd()
#
# car(20, 10, 6.5, 0, 0, 0)
#
